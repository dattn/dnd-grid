<template>
    <div class="dnd-grid-container" :style="style">
        <slot></slot>
        <box class="placeholder" boxId="::placeholder::"></box>
    </div>
</template>
...
export default {
    ...
    emits: ['update:layout', 'drag:start', 'drag:update', 'drag:end', 'resize:start', 'resize:update', 'resize:end'],
    ...
    provide () {
        return {
            [ContainerSymbol]: {
                ...
                unregisterBox: this.unregisterBox,
                isBoxVisible: this.isBoxVisible,
                getBoxPixelPosition: this.getBoxPixelPosition,
                onBoxDragStart: this.onBoxDragStart,
                onBoxDragEnd: this.onBoxDragEnd,
                onBoxDragUpdate: this.onBoxDragUpdate,
                onBoxResizeStart: this.onBoxResizeStart,
                onBoxResizeEnd: this.onBoxResizeEnd,
                onBoxResizeUpdate: this.onBoxResizeUpdate,
            }
        }
    },
    ...
    methods: {
        ...
        validateTargetPosition (targetX, targetY) {
        ...
        onBoxDragStart (box) {
            const { dragging } = this.$boxMap.get(box)
            ...
            // clone layout
            dragging.initialLayout = utils.sortLayout(this.layout)

            this.$emit('drag:start', dragging.initialLayout)
        },

        onBoxDragUpdate (box, data) {
            const { dragging } = this.$boxMap.get(box)

            this.dragging.offset.x = data.offset.x
            this.dragging.offset.y = data.offset.y

            var moveBy = this.getPositionByPixel(data.offset.x, data.offset.y)
            ...
            let { targetX, targetY } = this.validateTargetPosition(
            ...
            dragging.initialLayout.forEach((boxLayout) => {
        ...
        onBoxDragEnd (box, data) {
            ...
            dragging.initialLayout.forEach((boxPosition) => {
        ...
        validateTargetSize (targetW, targetH) {
        ...
        onBoxResizeStart (box) {
            const { resizing } = this.$boxMap.get(box)
            ...
            // clone layout
            resizing.initialLayout = utils.sortLayout(this.layout)

            this.$emit('resize:start', resizing.initialLayout)
        },

        onBoxResizeUpdate (box, data) {
            const { resizing } = this.$boxMap.get(box)

            this.resizing.offset.x = data.offset.x
            this.resizing.offset.y = data.offset.y

            var resizeBy = this.getPositionByPixel(data.offset.x, data.offset.y)
            if (!utils.isFree(this.pinnedLayout, {
            ...
            let { targetW, targetH } = this.validateTargetSize(
            ...
            var newLayout = [this.placeholder]
            resizing.initialLayout.forEach((boxLayout) => {
        ...
        onBoxResizeEnd (box, data) {
            ...
            resizing.initialLayout.forEach((boxPosition) => {
